   ...
   MAP
     MODULE('DATAABC_BC.CLW')
DctInit     PROCEDURE                                      ! Initializes the dictionary definition module
DctKill     PROCEDURE                                      ! Kills the dictionary definition module
     END
     MODULE('DATAABC001.CLW')
AppRoot                PROCEDURE   !ddd
     END
     INCLUDE('CWUtil.INC')
         module('w')
...
DLLInitializer.Construct PROCEDURE
  ...
  LocalINIMgr.Init('.\DataABC.INI', NVD_INI)               ! Initialize the local INI manager to use windows INI file
  INIMgr &= LocalINIMgr
...
DataABC:Init PROCEDURE(<ErrorClass curGlobalErrors>, <INIClass curINIMgr>)
DataABC:Init_Called    BYTE,STATIC

  CODE
  IF DataABC:Init_Called
     RETURN
  ELSE
     DataABC:Init_Called = True
  END
...
DataABC:Kill PROCEDURE
DataABC:Kill_Called    BYTE,STATIC

  CODE
  IF DataABC:Kill_Called
     RETURN
  ELSE
     DataABC:Kill_Called = True
  END
